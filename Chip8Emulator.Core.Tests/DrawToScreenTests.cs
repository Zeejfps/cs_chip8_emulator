...
public class DrawToScreenTests
{
    ...
    public void DrawsMultipleRows()
    {
        ...
        Chip8Routines.DrawToScreen(emulator.Cpu, 0xD003);
    ...
    public void DrawsAtCoordinatesFromVxVy()
    {
        ...
        Chip8Routines.SetIndexRegisterIns(emulator.Cpu, 0xA300);
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x620A); // V2 = 10
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x6305); // V3 = 5

        Chip8Routines.DrawToScreen(emulator.Cpu, 0xD231);
    ...
    public void StartCoordinateIsTakenModuloScreenSize()
    {
        ...
        emulator.Memory.Write(0x300, [0x80]);
        Chip8Routines.SetIndexRegisterIns(emulator.Cpu, 0xA300);
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x6246); // V2 = 70 -> x = 70 % 64 = 6
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x6328); // V3 = 40 -> y = 40 % 32 = 8
    ...
    {
        var emulator = CreateEmulator();
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x6F01); // VF = 1 from some earlier op
    ...
    public void DrawingSameSpriteTwiceErasesIt()
    {
        ...
        Chip8Routines.SetIndexRegisterIns(emulator.Cpu, 0xA300);
        Chip8Routines.DrawToScreen(emulator.Cpu, 0xD002);
    ...
    public void PartialOverlapSetsVfAndLeavesNonOverlappingBits()
    {
        ...
        Chip8Routines.SetIndexRegisterIns(emulator.Cpu, 0xA301);
        Chip8Routines.DrawToScreen(emulator.Cpu, 0xD001);
        Assert.Equal(0, emulator.Cpu.Registers.ReadV(0xF)); // no collision

        // Row 3: 0x81 = 10000001 — overlaps both existing bits
        emulator.Memory.Write(0x302, [0x81]);
        Chip8Routines.SetIndexRegisterIns(emulator.Cpu, 0xA302);
        Chip8Routines.DrawToScreen(emulator.Cpu, 0xD001);
        Assert.Equal(1, emulator.Cpu.Registers.ReadV(0xF)); // collision
    ...
    public void ClipsAtRightEdge()
    {
        ...
        emulator.Memory.Write(0x300, [0xFF]);
        Chip8Routines.SetIndexRegisterIns(emulator.Cpu, 0xA300);
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x623C); // V2 = 60 -> only 4 bits fit on screen
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x6300); // V3 = 0
    ...
    public void ClipsAtBottomEdge()
    {
        ...
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x631E); // V3 = 30 -> only rows 30,31 fit

        Chip8Routines.DrawToScreen(emulator.Cpu, 0xD234);
    ...
    {
        var emulator = CreateEmulator();
        emulator.Cpu.SpritesWrap = true;
        emulator.Memory.Write(0x300, [0xFF]);
        Chip8Routines.SetIndexRegisterIns(emulator.Cpu, 0xA300);
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x623C); // V2 = 60 -> bits 0..3 at x=60..63, bits 4..7 wrap to x=0..3
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x6300); // V3 = 0
    ...
    public void WrapsAtBottomEdgeWhenSpritesWrapIsOn()
    {
        ...
        Chip8Routines.SetIndexRegisterIns(emulator.Cpu, 0xA300);
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x6200); // V2 = 0
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x631E); // V3 = 30 -> rows at y=30,31, then wrap to y=0,1
    ...
    public void WrapsAtBothEdgesSimultaneouslyWhenSpritesWrapIsOn()
    {
        ...
        Chip8Routines.SetIndexRegisterIns(emulator.Cpu, 0xA300);
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x623F); // V2 = 63 -> x=63, then wrap to 0
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x631F); // V3 = 31 -> y=31, then wrap to 0

        Chip8Routines.DrawToScreen(emulator.Cpu, 0xD232);
    ...
    public void WrappedPixelSetsCollisionVf()
    {
        ...
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x6240); // V2 = 64 -> wraps to 0
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x6300);
        Chip8Routines.DrawToScreen(emulator.Cpu, 0xD231);
    ...
    {
        var emulator = CreateEmulator();
        Assert.False(emulator.Cpu.SpritesWrap);

        emulator.Memory.Write(0x300, [0xFF]);
        Chip8Routines.SetIndexRegisterIns(emulator.Cpu, 0xA300);
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x623C); // V2 = 60
        Chip8Routines.SetRegisterValue(emulator.Cpu, 0x6300);
    ...
    public void ReadsSpriteFromIndexRegister()
    {
        ...
        Chip8Routines.SetIndexRegisterIns(emulator.Cpu, 0xA050);
