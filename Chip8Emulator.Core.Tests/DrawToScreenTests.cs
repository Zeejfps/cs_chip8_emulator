...
public class DrawToScreenTests
{
    ...
    public void DrawsAtCoordinatesFromVxVy()
    {
        ...
        emulator.Step(0xA300);
        emulator.Step(0x620A); // V2 = 10
        emulator.Step(0x6305); // V3 = 5

        emulator.Step(0xD231);
    ...
    public void StartCoordinateIsTakenModuloScreenSize()
    {
        ...
        emulator.Memory.Write(0x300, [0x80]);
        emulator.Step(0xA300);
        emulator.Step(0x6246); // V2 = 70 -> x = 70 % 64 = 6
        emulator.Step(0x6328); // V3 = 40 -> y = 40 % 32 = 8
    ...
    {
        var emulator = CreateEmulator();
        emulator.Step(0x6F01); // VF = 1 from some earlier op
    ...
    public void ClipsAtRightEdge()
    {
        ...
        emulator.Memory.Write(0x300, [0xFF]);
        emulator.Step(0xA300);
        emulator.Step(0x623C); // V2 = 60 -> only 4 bits fit on screen
        emulator.Step(0x6300); // V3 = 0
    ...
    public void ClipsAtBottomEdge()
    {
        ...
        emulator.Step(0x631E); // V3 = 30 -> only rows 30,31 fit

        emulator.Step(0xD234);
    ...
    public void WrapsAtRightEdgeWhenSpritesWrapIsOn()
    {
        ...
        emulator.Memory.Write(0x300, [0xFF]);
        emulator.Step(0xA300);
        emulator.Step(0x623C); // V2 = 60 -> bits 0..3 at x=60..63, bits 4..7 wrap to x=0..3
        emulator.Step(0x6300); // V3 = 0
    ...
    public void WrapsAtBottomEdgeWhenSpritesWrapIsOn()
    {
        ...
        emulator.Step(0xA300);
        emulator.Step(0x6200); // V2 = 0
        emulator.Step(0x631E); // V3 = 30 -> rows at y=30,31, then wrap to y=0,1
    ...
    public void WrapsAtBothEdgesSimultaneouslyWhenSpritesWrapIsOn()
    {
        ...
        emulator.Step(0xA300);
        emulator.Step(0x623F); // V2 = 63 -> x=63, then wrap to 0
        emulator.Step(0x631F); // V3 = 31 -> y=31, then wrap to 0

        emulator.Step(0xD232);
    ...
    public void WrappedPixelSetsCollisionVf()
    {
        ...
        emulator.Step(0x6240); // V2 = 64 -> wraps to 0
        emulator.Step(0x6300);
        emulator.Step(0xD231);
