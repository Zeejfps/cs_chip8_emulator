...
public class XoChipTests
{
    ...
    public void SelectPlane_StoresPlaneMask()
    {
        ...
        Cpu.ExecuteTimerIns(emulator, 0xF201); // mask = 2 (plane 1 only)
    ...
    public void SelectPlane_ZeroMakesDrawsNoOp()
    {
        ...
        Cpu.ExecuteTimerIns(emulator, 0xF001); // mask = 0
    ...
    public void SelectPlane_PlaneOneDrawsIntoBit1()
    {
        ...
        Cpu.ExecuteTimerIns(emulator, 0xF201); // mask = 2
        Cpu.ExeuteDrawToScreenIns(emulator, 0xD001);
    ...
    public void SelectPlane_BothPlanesConsumeTwoByteRuns()
    {
        ...
        Cpu.ExecuteSetIndexRegisterIns(emulator, 0xA300);
        Cpu.ExecuteTimerIns(emulator, 0xF301); // mask = 3 (both planes)
    ...
    public void SelectPlane_BothPlanesCollisionVfOne()
    {
        ...
        Cpu.ExecuteSetIndexRegisterIns(emulator, 0xA300);
        Cpu.ExecuteTimerIns(emulator, 0xF301); // mask = 3
    ...
    public void Clear_OnlyClearsSelectedPlaneBits()
    {
        ...
        Cpu.ExecuteTimerIns(emulator, 0xF101); // mask = 1
        Cpu.ExecuteZeroBaseIns(emulator, 0x00E0);
    ...
    public void ScrollRight_OnlyMovesSelectedPlane()
    {
        ...
        Cpu.ExecuteTimerIns(emulator, 0xF201);
        Cpu.ExeuteDrawToScreenIns(emulator, 0xD001);
    ...
    public void LoadAudioPattern_CopiesSixteenBytesFromIndex()
    {
        ...
        Cpu.ExecuteTimerIns(emulator, 0xF002);
    ...
    public void LoadAudioPattern_IgnoredWhenXIsNonZero()
    {
        ...
        Cpu.ExecuteTimerIns(emulator, 0xF102); // F102 is not F002
    ...
    public void SetPitch_UpdatesFrequencyPerOctoSpec()
    {
        var audio = new FakeAudio();
        var emulator = CreateEmulator(audio);
        Cpu.ExecuteSetRegisterValueIns(emulator, 0x6070); // V0 = 112

        Cpu.ExecuteTimerIns(emulator, 0xF03A);
    ...
    public void SetPitch_DefaultPitch64Produces4000Hz()
    {
        var audio = new FakeAudio();
        var emulator = CreateEmulator(audio);
        Cpu.ExecuteSetRegisterValueIns(emulator, 0x6040); // V0 = 64
    ...
    public void SaveFlags_WritesV0ThroughVxToPersistentStorage()
    {
        ...
        Cpu.ExecuteSetRegisterValueIns(emulator, 0x60AA); // V0 = 0xAA
        Cpu.ExecuteSetRegisterValueIns(emulator, 0x61BB); // V1 = 0xBB
        Cpu.ExecuteSetRegisterValueIns(emulator, 0x62CC); // V2 = 0xCC

        Cpu.ExecuteTimerIns(emulator, 0xF275); // FX75 with X=2 saves V0..V2
    ...
    public void LoadFlags_RestoresV0ThroughVxFromPersistentStorage()
    {
        ...
        Cpu.ExecuteTimerIns(emulator, 0xF385); // FX85 with X=3 loads V0..V3
    ...
    public void SaveLoadFlags_RoundTripPreservesValues()
    {
        ...
        for (var i = 0; i < 16; i++)
        {
            Cpu.ExecuteSetRegisterValueIns(emulator, 0x6000 | (i << 8) | (i * 17));
        }

        Cpu.ExecuteTimerIns(emulator, 0xFF75); // save V0..VF

        var restored = CreateEmulator(flags);
        Cpu.ExecuteTimerIns(restored, 0xFF85); // load V0..VF
    ...
    public void LongLoadI_ReadsNextTwoBytesIntoIndexRegister()
    {
        ...
        Cpu.ExecuteTimerIns(emulator, 0xF000);
